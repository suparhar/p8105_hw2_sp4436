...
After merging, the combined dataset contains `r nrow(merged_data)` rows and `r ncol(merged_data)` columns. Each row corresponds to a unique year–month, and the variables include political party composition, the president’s party, S&P closing values, and unemployment rates. The merged dataset spans the years `r min(merged_data$year)` through `r max(merged_data$year)`.
